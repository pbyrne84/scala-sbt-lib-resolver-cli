    ...
    suite(getClass.getSimpleName)(
      suite("fromCliArgs")(
        test("should parse a hotList command line args with no debug") {
          ...
          val expected = Right(CommandLineArgs(HotListLookupType("test-single"), "internal", enableDebug = false))
        ...
        test("should parse a hotList command line args with debug logging") {
          ...
          val expected = Right(CommandLineArgs(HotListLookupType("test-single"), "internal", enableDebug = true))
          val actual = CommandLineArgs.fromCliArgs(args)
